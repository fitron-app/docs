...
| 配置项 | 说明 | 默认值 |
|---|---|---|
| 月度打卡目标 | 每月建议打卡天数 | 12 天 |
| 打卡奖励 | 达到目标后是否发放优惠券 | 关闭（可开启） |
| 奖励优惠券 | 达标后自动发放的优惠券模板 | 无 |
| 目标达成提示 | 达标后的提示文案 | 「恭喜！本月已打卡 X 天，达成目标🎉」 |
...
用户进入运动日历
  → 请求本月打卡统计
  → 本月打卡天数 ≥ 月度目标
  → 且本月尚未发放奖励（检查 localRewardClaimed_{month}）
  → 弹出「达标恭喜」卡片
  → 「领取奖励」→ 自动发放优惠券
  → 「继续加油」→ 关闭
```
...
GET /api/v1/user/workout-calendar
  ...
  Response: {
    ...
    monthlyGoal: {
      target: number,           // 月度目标天数
      achieved: boolean,        // 是否已达标
      rewardClaimed: boolean    // 奖励是否已领取
    }
  }

# 领取打卡奖励
POST /api/v1/user/workout-reward
  Auth: JWT
  Body: { year: number, month: number }
  Response: {
    success: boolean,
    coupon: { name, discountDesc, validDays } | null
  }
